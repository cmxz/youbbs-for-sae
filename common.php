...
include (ROOT . '/model.php');

// 获得散列
function formhash() {
    global $cur_ucode, $options;
    return substr(md5($options['site_create'].$cur_ucode.'your_secret_words_here'), 8, 8);
}
...
if($options['authorized'] && (!$cur_user || $cur_user['flag']<5)){
    if( !in_array($url_path, array('login','logout','sigin','forgot','fenci'))){
...
function set_content($text,$spider='0'){
    ...
    if(preg_match($img_re, $text)){
        if(!$spider){
            $text = preg_replace($img_re, '<a href="\1" target="_blank"><img src="'.$options['base_url'].'/static/grey2.gif" data-original="\1" alt="" /></a>', $text);
        }else{
    ...
    if(strpos($text, 'qpic.cn')){
        ...
        if(!$spider){
            $text = preg_replace($qq_img_re, '<a href="\1/2000" target="_blank"><img src="'.$options['base_url'].'/static/grey2.gif" data-original="\1/460" alt="" /></a>', $text);
        }else{
            // 搜索引擎来这样显示 更利于SEO 参见 http://saepy.sinaapp.com/t/81
    ...
    // mentions
    if(strpos(' '.$text, '@')){
        $text = preg_replace('/\B\@([a-zA-Z0-9\x80-\xff]{1,20})\s?/', ' @<a href="'.$options['base_url'].'/member/\1">\1</a> ', $text);
    }
    ...
    if(strpos(' '.$text, 'http')){
        ...
        $text = substr($text, 1);
        $text = str_replace('target="_blank" rel="nofollow" href="http://youbbs.sinaapp.com', 'href="http://youbbs.sinaapp.com', $text);
    }
...
function find_mentions($text, $filter_name='', $filter_id=''){
    // 正则跟用户注册、登录保持一致
    preg_match_all('/\B\@([a-zA-Z0-9\x80-\xff]{1,20})\s?/' ,$text, $out, PREG_PATTERN_ORDER);
    $new_arr = array_unique($out[1]);
    if(in_array($filter_name, $new_arr) || in_array($filter_id, $new_arr)){
